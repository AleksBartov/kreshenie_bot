...
async function initializeYandexDisk() {
  try {
    console.log('🔐 Проверка токена Яндекс.Диска...');
    ...
    if (tokenCheck.success) {
      ...
      await yandexDisk.ensureFolderStructure('/interview_bot');
    } else {
      yandexDiskEnabled = false;
      console.log('❌ Яндекс.Диск: Токен невалиден -', tokenCheck.error);
    }
  } catch (error) {
    yandexDiskEnabled = false;
    console.log('❌ Яндекс.Диск: Ошибка инициализации -', error.message);
  }
}
