...
console.log('🔐 Токен бота:', process.env.BOT_TOKEN ? 'присутствует' : 'ОТСУТСТВУЕТ');
console.log('🔐 Токен Яндекс:', process.env.YANDEX_ACCESS_TOKEN ? `(длина: ${process.env.YANDEX_ACCESS_TOKEN.length})` : 'ОТСУТСТВУЕТ');
...
async function initializeYandexDisk() {
  try {
    console.log('\n🔐 ===== ИНИЦИАЛИЗАЦИЯ ЯНДЕКС.ДИСК =====');
    
    const tokenCheck = await yandexDisk.checkToken();
    console.log('🔐 Результат проверки токена:', tokenCheck);
    ...
    if (tokenCheck.success) {
      ...
      console.log('📁 Тестируем создание папки...');
      const folderTest = await yandexDisk.createFolder('/interview_bot');
      console.log('📁 Результат создания папки:', folderTest);
      
    } else {
      yandexDiskEnabled = false;
      console.log('❌ Яндекс.Диск: Токен невалиден');
    }
    
    console.log('💾 Яндекс.Диск включен:', yandexDiskEnabled);
    console.log('🔐 ===== ИНИЦИАЛИЗАЦИЯ ЗАВЕРШЕНА =====\n');
    
  } catch (error) {
    yandexDiskEnabled = false;
    console.log('❌ Яндекс.Диск: Критическая ошибка инициализации -', error.message);
  }
}
...
try {
  ...
  console.log('📝 Загружено блоков:', interviewBlocks.length);
} catch (error) {
...
async function saveVoiceMessage(ctx, userFolderInfo, fileName) {
  ...
  try {
    ...
    if (yandexDiskEnabled) {
      console.log('☁️  Попытка загрузки на Яндекс.Диск...');
      
      // Убедимся что папка создана
      if (!ctx.session.yandexFolderCreated) {
        console.log('📁 Создание папки на Яндекс.Диске...');
        const folderResult = await yandexDisk.createFolder(yandexFolderPath);
        ctx.session.yandexFolderCreated = folderResult.success;
        console.log('📁 Результат создания папки:', folderResult);
      }
      
      if (ctx.session.yandexFolderCreated) {
        try {
          const remoteFilePath = `${yandexFolderPath}/${fileName}`;
          console.log(`☁️  Загрузка на Яндекс.Диск: ${remoteFilePath}`);
          const uploadResult = await yandexDisk.uploadFile(localFilePath, remoteFilePath);
          uploadedToYandex = uploadResult.success;
          yandexError = uploadResult.error;
          console.log(`☁️  Результат загрузки: ${uploadedToYandex ? 'УСПЕХ' : 'ОШИБКА'}`);
        } catch (error) {
          console.error('☁️  Ошибка загрузки на Яндекс.Диск:', error);
          uploadedToYandex = false;
          yandexError = error.message;
        }
      } else {
        console.log('❌ Папка на Яндекс.Диске не создана, пропускаем загрузку');
      }
    } else {
      console.log('💾 Яндекс.Диск отключен, сохраняем только локально');
    }
    ...
    console.log(`✅ Метаданные сохранены для: ${fileName}`);
...
async function completeInterview(ctx) {
  ...
  console.log(`✅ Завершение собеседования для пользователя: ${session.username}`);
  ...
  if (yandexDiskEnabled && session.yandexFolderCreated) {
    try {
      console.log('🔗 Публикация папки на Яндекс.Диске...');
      const publishResult = await yandexDisk.publishFolder(session.yandexFolderPath);
      if (publishResult.success) {
        folderUrl = publishResult.publicUrl;
        console.log('✅ Папка опубликована:', folderUrl);
      } else {
        folderUrl = publishResult.publicUrl || session.yandexFolderPath;
        console.log('⚠️  Папка не опубликована, используем путь:', folderUrl);
      }
    } catch (error) {
      console.error('❌ Ошибка публикации папки:', error);
      folderUrl = session.yandexFolderPath;
    }
  }
...
async function notifyAdmin(ctx, folderUrl) {
  ...
  console.log(`📢 Уведомление отправлено админу: ${session.username}`);
}
...
// Команда /start
bot.start(async (ctx) => {
  console.log(`🚀 /start от пользователя: ${ctx.from.username} (ID: ${ctx.from.id})`);
  ...
  console.log(`📁 Создана папка пользователя: ${userFolderInfo.localFolderPath}`);
