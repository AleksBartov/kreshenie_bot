...
async function getGrades(periodName, dateFrom, dateTo) {
  try {
    ...
    const url = createExactUrl(IDes.Varvara, dateFrom, dateTo, 200, 1);
      ...
      Object.keys(byDate)
        ...
        .forEach(date => {
          byDate[date].forEach(grade => {
            ...
            return `   📚 ${subject} ${grade.estimate_value_name} (${type})`
          });
        });
...
today.setDate(today.getDate()-1)
  ...
  await getGrades('ВЧЕРА', yesterday, yesterday);
}
...
bot.action('varvara', async(ctx, next)=> {
    const grades = await getTodayGrades();
    ctx.answerCbQuery()
    ctx.reply(grades)
})
